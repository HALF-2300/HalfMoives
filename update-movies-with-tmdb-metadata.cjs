...
async function updateMovies() {
  console.log(`\n📚 Updating ${moviesToUpdate.length} movies with TMDB metadata...\n`);
  ...
  for (let i = 0; i < moviesToUpdate.length; i++) {
    ...
    if (!movie) {
      console.log(`⏭️  [${i+1}/${moviesToUpdate.length}] Not found in database: ${movieInfo.title} (${movieInfo.year})`);
      notFound++;
      continue;
    }
    
    // Skip if already has good poster (not placeholder)
    if (movie.posterUrl && 
        !movie.posterUrl.includes('placehold.co') && 
        !movie.posterUrl.includes('img.youtube.com') &&
        movie.posterUrl.includes('tmdb.org')) {
      console.log(`⏭️  [${i+1}/${moviesToUpdate.length}] Already has TMDB poster: ${movie.title}`);
      skipped++;
      continue;
    }
    
    try {
      console.log(`📥 [${i+1}/${moviesToUpdate.length}] Fetching: ${movieInfo.title} (${movieInfo.year})...`);
      ...
      // Update movie
      movie.title = details.title || movieInfo.title;
      ...
      if (details.poster_path) {
        movie.posterUrl = `${TMDB_IMAGE_BASE}${details.poster_path}`;
        movie.thumbnailUrl = `${TMDB_IMAGE_BASE}${details.poster_path}`;
      }
      
      if (details.genres && details.genres.length > 0) {
        movie.genres = details.genres.map(g => g.name);
        movie.tags = details.genres.map(g => g.name.toLowerCase());
      } else if (movieInfo.genres && movieInfo.genres.length > 0) {
